  ...
  return (
    <div className="mb-10 flex flex-wrap items-start justify-between gap-6">
      <div className="max-w-3xl">
        {eyebrow && (
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#806945]">
            {eyebrow}
          </p>
        )}
...
export function MetricCard({
  ...
  return (
    <div className="rounded-xl border border-[#d6cbbb] bg-[#f7f1e8] p-5 shadow-[0_12px_35px_rgba(74,63,48,0.05)]">
      <div className="text-[11px] font-medium uppercase tracking-[0.16em] text-[#6f685e]">
        {title}
      </div>
      <p className="mt-3 font-serif text-[2rem] font-medium leading-none tracking-tight text-[#2f2b24] tabular-nums">
        {value}
      </p>
      {hint && <p className="mt-3 text-xs leading-5 text-[#6f685e]">{hint}</p>}
    </div>
  )
}
...
export function DemoNotice({ children }: { children: ReactNode }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-[#cdbfa9] bg-[#f2eadf] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#806945]">
      Demo
      <span className="font-normal normal-case tracking-normal text-[#665f56]">
        {children}
      </span>
    </span>
  )
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 text-sm">
        <span className="text-[#5f5951]">{label}</span>
