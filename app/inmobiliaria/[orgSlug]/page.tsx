...
async function loadSite(orgSlug: string) {
  ...
  const { data: site } = await supabase.from("brokerage_public_sites").select("*").eq("site_slug", orgSlug).eq("is_active", true).maybeSingle();
  if (!site) return null;
...
export async function generateMetadata({ params }: { params: Promise<{ orgSlug: string }> }): Promise<Metadata> {
  const { orgSlug } = await params; const data = await loadSite(orgSlug);
  if (!data) return { title: "Inmobiliaria no disponible" };
  const title = data.site.seo_title || "Propiedades"; const description = data.site.seo_description || data.site.tagline || "Propiedades disponibles";
  return { title, description, openGraph: { title, description, type: "website", images: data.site.hero_image_url ? [data.site.hero_image_url] : undefined } };
}
...
export default async function BrokerageSite({ params, searchParams }: { params: Promise<{ orgSlug: string }>; searchParams: Promise<{ operation?: string; type?: string; zone?: string }> }) {
  const { orgSlug } = await params; const filters = await searchParams; const data = await loadSite(orgSlug); if (!data) notFound();
  ...
  return <main className="min-h-screen bg-[#f3eadf] text-[#302d28]" style={{ ["--brand" as string]: site.accent_color || "#302d28" }}>
    <header className="sticky top-0 z-20 border-b border-[#d8cbb8] bg-[#f8f1e8]/95 px-5 py-4 backdrop-blur md:px-8"><div className="mx-auto flex max-w-7xl items-center justify-between gap-5">
      <Link href={`/inmobiliaria/${orgSlug}`} className="flex items-center gap-3">{site.logo_url ? <img src={site.logo_url} alt="Logo" className="h-10 max-w-40 object-contain"/> : <span className="flex h-10 w-10 items-center justify-center rounded-full border border-[#cdbfa9]"><Building2 size={18}/></span>}<span className="font-serif text-xl">{site.seo_title?.split("|")[0]?.trim() || "Inmobiliaria"}</span></Link>
      <div className="flex items-center gap-2">{whatsapp && <a href={`https://wa.me/${whatsapp}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-[#cdbfa9] px-3 py-2 text-sm font-semibold"><MessageCircle size={15}/> WhatsApp</a>}<a href="#contacto" className="rounded-lg bg-[var(--brand)] px-4 py-2 text-sm font-semibold !text-white">Consultar</a></div>
    </div></header>

    <section className="relative overflow-hidden border-b border-[#d8cbb8] bg-[#e9dfd0]">{site.hero_image_url && <div className="absolute inset-0 bg-cover bg-center opacity-35" style={{ backgroundImage: `url(${site.hero_image_url})` }}/>}<div className="absolute inset-0 bg-gradient-to-r from-[#f3eadf] via-[#f3eadf]/90 to-[#f3eadf]/35"/><div className="relative mx-auto max-w-7xl px-5 py-16 md:px-8 md:py-24"><p className="text-[10px] font-semibold uppercase tracking-[.2em] text-[#8d7553]">Propiedades</p><h1 className="mt-3 max-w-3xl font-serif text-5xl leading-[1.02] md:text-7xl">{site.tagline || "Encontrá tu próxima propiedad."}</h1>{site.about && <p className="mt-6 max-w-2xl text-sm leading-7 text-[#625d55] md:text-base">{site.about}</p>}<p className="mt-7 text-sm font-semibold text-[#6a5a45]">{publications.length} propiedades publicadas</p></div></section>

    <div className="mx-auto max-w-7xl px-5 py-8 md:px-8 md:py-12">
      <form className="mb-8 grid gap-3 rounded-2xl border border-[#d2c5b3] bg-[#f7f0e6] p-4 md:grid-cols-[1fr_1fr_1fr_auto]"><Select name="operation" label="Operación" value={filters.operation} options={operations}/><Select name="type" label="Tipo" value={filters.type} options={types}/><Select name="zone" label="Zona" value={filters.zone} options={zones}/><button className="mt-auto inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-[var(--brand)] px-5 text-sm font-semibold text-white"><Search size={15}/> Buscar</button></form>
      {visible.length ? <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{visible.map((p) => <Link href={`/p/${p.public_slug}`} key={p.public_slug} className="group overflow-hidden rounded-2xl border border-[#d2c5b3] bg-[#f7f0e6] shadow-[0_14px_30px_rgba(76,62,42,.05)]"><div className="relative aspect-[16/10] bg-[#ded2c1] bg-cover bg-center" style={p.cover_image_url ? { backgroundImage: `url(${p.cover_image_url})` } : undefined}>{!p.cover_image_url && <div className="absolute inset-0 flex items-center justify-center text-[#8a7a67]"><Building2 size={44} strokeWidth={1.2}/></div>}<div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent"/><span className="absolute left-4 top-4 rounded-full border border-white/30 bg-black/30 px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-white">{p.operation || "Propiedad"}</span></div><div className="p-5"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#8d7553]">{p.zone || "Uruguay"}</p><h2 className="mt-2 font-serif text-2xl">{p.title}</h2>{p.address_label && <p className="mt-2 flex items-center gap-1.5 text-xs text-[#81786d]"><MapPin size={13}/>{p.address_label}</p>}<p className="mt-5 font-serif text-2xl">{p.price != null ? `${p.currency || ""} ${Number(p.price).toLocaleString()}` : "Consultar"}</p><p className="mt-2 text-xs text-[#81786d]">{p.bedrooms ?? "—"} dorm. · {p.bathrooms ?? "—"} baños · {p.area_m2 ? `${Number(p.area_m2)} m²` : "Área a consultar"}</p></div></Link>)}</section> : <div className="rounded-2xl border border-[#d2c5b3] bg-[#f7f0e6] p-10 text-center"><p className="font-serif text-2xl">No encontramos propiedades con esos filtros.</p><Link href={`/inmobiliaria/${orgSlug}`} className="mt-3 inline-block text-sm font-semibold text-[#6a5a45]">Ver todo el catálogo</Link></div>}
      <section id="contacto" className="mt-14 grid gap-7 rounded-3xl border border-[#d2c5b3] bg-[#e8dccb] p-6 md:grid-cols-[.8fr_1.2fr] md:p-9"><div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[#8d7553]">Contacto</p><h2 className="mt-3 font-serif text-4xl">Contanos qué estás buscando.</h2><p className="mt-4 text-sm leading-6 text-[#665f56]">La consulta llega directamente al equipo comercial de esta inmobiliaria. No se comparte con otras inmobiliarias de RevScale.</p><div className="mt-6 space-y-2 text-sm text-[#665f56]">{site.public_phone && <p className="flex gap-2"><Phone size={15}/>{site.public_phone}</p>}{site.public_email && <p className="flex gap-2"><Mail size={15}/>{site.public_email}</p>}{site.public_address && <p className="flex gap-2"><MapPin size={15}/>{site.public_address}</p>}</div><div className="mt-5 flex gap-3">{site.instagram_url && <a href={site.instagram_url} target="_blank" rel="noreferrer"><Instagram size={18}/></a>}{site.facebook_url && <a href={site.facebook_url} target="_blank" rel="noreferrer"><Facebook size={18}/></a>}</div></div><PublicSiteInquiryForm siteSlug={site.site_slug}/></section>
    </div><footer className="border-t border-[#d8cbb8] px-5 py-7 text-center text-xs text-[#81786d]">{site.hide_revscale_branding ? "Propiedades y contacto directo" : "Sitio inmobiliario operado con RevScale PropertyOS"}</footer>
  </main>;
}
