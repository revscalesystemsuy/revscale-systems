...
async function getPublication(slug: string) {
  ...
  const { data: site } = await supabase.from("brokerage_public_sites").select("site_slug,is_active,logo_url,public_whatsapp,public_phone,hide_revscale_branding,seo_title,lead_capture_enabled").eq("organization_id", property.organization_id).eq("is_active", true).maybeSingle();
  if (!site) return null;
...
export default async function PublicPropertyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const displayName = site.seo_title?.split("|")[0]?.trim() || property.organization_name;
  return <main className="min-h-screen bg-[#f3eadf] text-[#302d28]">
    <header className="border-b border-[#d8cbb8] bg-[#f8f1e8] px-5 py-4 md:px-8"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4"><Link href={`/inmobiliaria/${site.site_slug}`} className="flex items-center gap-3">{site.logo_url ? <img src={site.logo_url} alt={`Logo de ${displayName}`} className="h-9 max-w-36 object-contain"/> : <span className="flex h-9 w-9 items-center justify-center rounded-full border border-[#cdbfa9]"><Building2 size={18}/></span>}<div><span className="block font-serif text-xl">{displayName}</span><span className="text-[9px] font-semibold uppercase tracking-[.16em] text-[#8d7553]">Catálogo inmobiliario</span></div></Link>{!site.hide_revscale_branding && <span className="text-[10px] font-semibold uppercase tracking-[.18em] text-[#8d7553]">Powered by RevScale</span>}</div></header>
