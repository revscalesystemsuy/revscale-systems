...
export default async function InternalSalesPipelinePage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const { data } = await supabase.from("b2b_opportunities").select("id,source_type,company,contact_name,email,phone,source_status,stage,primary_channel,plan_interest,next_step,next_step_due_at,last_contact_at,icp_score,tier,created_at").order("created_at", { ascending: false });
