...
export default async function CommissionsPage() {
  ...
  const [
    ...
    context.supabase.from("commissions").select("id,lead_id,agent_id,operation,currency,deal_amount,deal_amount_source,brokerage_rate,gross_commission,agent_split_rate,agent_commission,office_commission,collected_amount,payment_status,due_date,notes,created_at").eq("organization_id", orgId).order("created_at", { ascending: false }),
    ...
    context.supabase.from("leads").select("id,full_name").eq("organization_id", orgId),
  ...
  const leadNameById = new Map((leadsData || []).map((lead) => [lead.id, lead.full_name || "Lead sin nombre"]));
  const profileNameById = new Map((profilesData || []).map((profile) => [profile.id, profile.full_name]));
  const agents = (membersData || []).filter((member) => member.role === "AGENT").map((member) => ({ id: member.user_id, name: profileNameById.get(member.user_id) || "Agente" }));
    ...
    <main className="min-h-screen p-6 md:p-8 lg:p-10">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[#8d7553]">Dirección comercial</p>
            <h1 className="mt-3 font-serif text-4xl font-medium tracking-tight text-[#292722] md:text-5xl">Comisiones</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[#625d55] md:text-[15px]">Cada cierre ganado genera una liquidación. Ajustá el valor real, el porcentaje de honorarios, el split del agente y el estado de cobro.</p>
          </div>
          <div className="rounded-xl border border-[#cdbfa9] bg-[#f7f0e6] px-4 py-3 text-sm text-[#655842]"><span className="font-semibold">Automático</span> · se crea al pasar una oportunidad a Cierre</div>
        </div>
        ...
        {(commissionsError || rulesError) && <div className="mt-6 rounded-xl border border-[#d3b7ae] bg-[#f1dfd8] p-4 text-sm text-[#704b3d]">No se pudo cargar una parte del módulo de comisiones.</div>}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="mb-1.5 block text-xs font-semibold text-[#655d53]">{label}</span>{children}</label>; }
...
function StatusBadge({ value }: { value: string }) { const cls = value === "PAID" ? "border-[#b8c1a4] bg-[#e7eadf] text-[#596146]" : value === "CANCELLED" ? "border-[#ccb7ae] bg-[#eee0db] text-[#775247]" : value === "PARTIAL" ? "border-[#c8b58d] bg-[#eee5ce] text-[#735f34]" : "border-[#cfc2b1] bg-[#f3ece2] text-[#756b61]"; return <span className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold ${cls}`}>{STATUS_COPY[value] || value}</span>; }
