...
import { ExternalLink, Globe2, Link2, LockKeyhole, Search, ShieldCheck } from "lucide-react";
...
export default async function PublicSiteSettingsPage() {
  ...
  return <main className="min-h-screen p-5 md:p-8 lg:p-10"><div className="mx-auto max-w-[1250px]">
    <div className="mb-7 flex flex-col gap-4 border-b border-[#d8cbb8] pb-6 md:flex-row md:items-end md:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-[#8d7553]">Distribución · Catálogo público</p><h1 className="mt-2 font-serif text-4xl text-[#302b25]">Datos del sitio público</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[#665f56]">El catálogo usa la plantilla visual oficial de RevScale. La inmobiliaria administra su identidad comercial, contacto, propiedades y SEO, pero no modifica colores, estructura ni diseño.</p></div><div className="flex gap-2"><Link href="/protected/distribution" className="rounded-lg border border-[#cdbfa9] px-4 py-2.5 text-sm font-semibold text-[#5f513e]">Volver</Link>{siteReady ? <a href={publicUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-[#302d28] px-4 py-2.5 text-sm font-semibold !text-[#fffaf2]">Ver catálogo público <ExternalLink size={14}/></a> : <span className="inline-flex items-center gap-2 rounded-lg border border-[#d2c5b3] bg-[#efe5d7] px-4 py-2.5 text-sm font-semibold text-[#81786d]">Catálogo desactivado</span>}</div></div>

    <section className="mb-6 rounded-2xl border border-[#cdbfa9] bg-[#e8dccb] p-5 md:p-6"><div className="flex items-start gap-3"><ShieldCheck size={20} className="mt-0.5 shrink-0 text-[#6f5d45]"/><div><h2 className="font-serif text-2xl text-[#302b25]">Una sola experiencia pública RevScale</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-[#665f56]">Todos los catálogos comparten la misma estética sobria y profesional del sistema. Solo cambian el nombre, logo, textos, datos de contacto y propiedades de cada inmobiliaria. Así mantenemos calidad visual y evitamos sitios inconsistentes.</p></div></div></section>
    ...
    <div className="mb-6">{siteReady ? <PublicSiteSharePanel url={publicUrl}/> : <section className="rounded-2xl border border-dashed border-[#cdbfa9] bg-[#f7f0e6] p-5 text-sm text-[#716a61]">Activá el catálogo para habilitar enlace público, compartir y código QR.</section>}</div>

    <form action={savePublicSite} className="grid gap-6 lg:grid-cols-[1.3fr_.7fr]">
      <div className="space-y-6">
        <Panel title="Identidad de la inmobiliaria" icon={<Globe2 size={17}/>}>
          <Field label="Eslogan"><input name="tagline" maxLength={180} defaultValue={site?.tagline || "Propiedades seleccionadas y atención personalizada."}/></Field>
          <Field label="Quiénes somos"><textarea name="about" rows={5} maxLength={1800} defaultValue={site?.about || ""}/></Field>
          <Field label="URL del logo"><input name="logo_url" type="url" defaultValue={site?.logo_url || ""}/></Field>
          <p className="text-xs leading-5 text-[#81786d]">El logo identifica a la inmobiliaria; la composición, tipografía, colores y portada del catálogo son gestionados por RevScale.</p>
        </Panel>
      ...
      <div className="space-y-6">
        <Panel title="Publicación" icon={<Globe2 size={17}/>}><Toggle name="is_active" label="Catálogo público activo" checked={site?.is_active ?? true}/><Toggle name="lead_capture_enabled" label="Capturar consultas en CRM" checked={site?.lead_capture_enabled ?? true}/><p className="text-xs leading-5 text-[#81786d]">Solo las propiedades marcadas como Publicadas en Distribución aparecen afuera del CRM.</p></Panel>
        <Panel title="Dominio y white-label" icon={isEnterprise ? <Globe2 size={17}/> : <LockKeyhole size={17}/>}>
          {isEnterprise ? <><Field label="Dominio propio"><input name="custom_domain" placeholder="propiedades.tuinmobiliaria.com.uy" defaultValue={site?.custom_domain || ""}/></Field><p className="text-xs leading-5 text-[#81786d]">Estado: <strong>{site?.custom_domain_status || "NOT_CONFIGURED"}</strong>. El dominio cambia la dirección pública, no el diseño del catálogo.</p><Toggle name="hide_revscale_branding" label="Ocultar Powered by RevScale" checked={site?.hide_revscale_branding || false}/></> : <div className="rounded-xl border border-[#d2c5b3] bg-[#efe5d7] p-4"><p className="text-sm font-semibold">Disponible en Enterprise</p><p className="mt-2 text-xs leading-5 text-[#716a61]">Professional incluye el catálogo completo con URL RevScale. Enterprise suma dominio propio y white-label, manteniendo la misma plantilla visual.</p></div>}
        </Panel>
        <button className="w-full rounded-lg bg-[#302d28] px-5 py-3 text-sm font-semibold text-[#fffaf2]">Guardar datos del catálogo</button>
      </div>
    </form>
  </div></main>;
}
