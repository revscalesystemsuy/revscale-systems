  ...
  return (
    <main className="min-h-screen p-6 md:p-8 lg:p-10">
      <div className="mx-auto max-w-6xl">
        <Link href="/protected" className="text-sm font-medium text-[#7a674c] transition hover:text-[#302d28]">
          ← Dashboard
        </Link>
        ...
        <div className="mt-6">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-[#625d55] md:text-[15px]">
            Importá leads y propiedades desde archivos CSV sin cargarlos uno por uno. RevScale valida el archivo, evita duplicados y respeta los límites de tu plan.
          </p>
        </div>

        {error && (
          <div className="mt-6 rounded-xl border border-[#d9b7aa] bg-[#f4e4dc] p-4 text-sm text-[#7b4539]">{error}</div>
        )}
        ...
        <section className="mt-8 rounded-xl border border-[#cdbfa9] bg-[#eee4d5] p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-[#5f584f]">
            Descargá la plantilla, completala en Excel o Google Sheets y exportala como CSV. Se aceptan archivos separados por coma, punto y coma o tabulación. Si alguna fila tiene un dato inválido, RevScale no realiza la importación hasta que lo corrijas. Los duplicados detectados se omiten automáticamente.
          </p>
  ...
  return (
    <article className="rounded-xl border border-[#d2c5b3] bg-[#f7f0e6] p-6 shadow-[0_18px_45px_rgba(72,58,40,0.04)]">
      <div className="flex items-start gap-3">
        <div className="rounded-lg border border-[#cdbfa9] bg-[#eee4d5] p-2.5 text-[#705f47]">{icon}</div>
        <div>
          <h2 className="font-serif text-xl font-medium text-[#37332d]">{title}</h2>
          <p className="mt-1 text-sm leading-6 text-[#665f56]">{description}</p>
        </div>
      </div>

      <div className="mt-5 rounded-xl border border-[#ddd1c0] bg-[#fffaf2] p-4">
        <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#8a7a67]">Columnas aceptadas</p>
        <p className="mt-2 text-sm leading-6 text-[#49433b]">{columns}</p>
      </div>

      <a href={templateHref} download className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[#725d40] transition hover:text-[#3f3529]">
        <Download size={16} strokeWidth={1.7} /> Descargar plantilla CSV
      </a>

      <form action={action} className="mt-5 space-y-4">
        <label className="block rounded-xl border border-dashed border-[#cbbca8] bg-[#fffaf2] p-4">
          <span className="mb-3 flex items-center gap-2 text-sm font-medium text-[#514a42]"><Upload size={16} /> Seleccionar archivo CSV</span>
          <input
            name="file"
            type="file"
            accept=".csv,text/csv"
            required
            className="block w-full text-sm text-[#5f584f] file:mr-4 file:rounded-lg file:border-0 file:bg-[#e7dccb] file:px-4 file:py-2 file:font-semibold file:text-[#3f392f] hover:file:bg-[#ddd0bd]"
          />
        </label>
        <button className="w-full rounded-lg bg-[#302d28] px-5 py-3 text-sm font-semibold !text-[#fffaf2] transition hover:bg-[#3b3731]">
          Validar e importar
        </button>
      </form>
    </article>
