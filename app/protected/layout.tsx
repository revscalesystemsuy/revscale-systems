      ...
      <div className="flex min-h-screen items-center justify-center bg-[#eee5d7] p-6 text-[#292722]">
        <div className="w-full max-w-xl rounded-2xl border border-[#d3c6b3] bg-[#f7f0e6] p-8 text-center shadow-[0_24px_70px_rgba(71,58,40,0.08)]">
          ...
          <p className="mt-4 leading-6 text-[#625d55]">
            El acceso a RevScale PropertyOS está temporalmente suspendido. Tus leads, propiedades, historial, usuarios y configuración se conservan sin cambios.
          </p>
          ...
          <a href="https://wa.me/59892715418" target="_blank" className="mt-7 inline-block rounded-lg bg-[#292722] px-5 py-3 font-semibold text-[#f8f1e7] transition hover:bg-[#3a3731]">
  ...
  return (
    <div className="revscale-real-app flex min-h-screen bg-[#eee5d7] text-[#292722]">
      <aside className="sticky top-0 flex h-screen w-72 shrink-0 flex-col border-r border-[#d2c6b5] bg-[#e5dac9] px-5 py-6">
        <div className="px-2">
          <p className="font-serif text-[1.45rem] leading-none tracking-tight text-[#292722]">RevScale</p>
          <p className="mt-2 text-[10px] font-semibold uppercase tracking-[0.24em] text-[#927a58]">PropertyOS</p>
        </div>

        <div className="mx-2 mt-7 border-y border-[#d1c4b1] py-4">
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#81796e]">Plan actual</p>
          <p className="mt-2 text-sm font-semibold text-[#4b453d]">{plan}</p>
          {enterprise && (
            <div className="mt-3 border-t border-[#d1c4b1] pt-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#81796e]">Tu rol</p>
        ...
        <div className="mx-2 mt-4 border-t border-[#d1c4b1] pt-4">
          <form action={signOut}>
            <button type="submit" className="group flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-[#675e53] transition hover:bg-[#f0e8dc] hover:text-[#292722]">
              <LogOut size={16} strokeWidth={1.6} className="text-[#8b7d69] group-hover:text-[#6d5b43]" />
              Cerrar sesión
            </button>
          </form>
          <div className="mt-4 border-t border-[#d1c4b1] pt-4">
...
function NavItem({ href, children, icon, locked = false, badge = 0 }: { href: string; children: ReactNode; icon?: ReactNode; locked?: boolean; badge?: number }) {
  return (
    <Link href={href} className="group flex items-center justify-between rounded-lg px-3 py-2.5 text-sm text-[#5f594f] transition hover:bg-[#f0e8dc] hover:text-[#292722]">
      <span className="flex min-w-0 items-center gap-3">
        <span className="text-[#8b7d69] transition group-hover:text-[#6d5b43]">{icon}</span>
        <span>{children}</span>
      </span>
      <span className="flex items-center gap-2">
        {badge > 0 && <span className="min-w-5 rounded-full bg-[#6f5c40] px-1.5 py-0.5 text-center text-[10px] font-bold text-[#fffaf2]">{badge > 99 ? "99+" : badge}</span>}
