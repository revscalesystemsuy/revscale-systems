...
const STAGE_CLASS: Record<string, string> = {
  NEW: "border-[#d8c9b5] bg-[#eee4d5] text-[#705f48]", CONTACTED: "border-[#c9c9b9] bg-[#e9e8dc] text-[#5f6254]", QUALIFIED: "border-[#c7cdb8] bg-[#e9ecdf] text-[#596149]", VISIT: "border-[#c8c1d0] bg-[#ece8ef] text-[#62566b]", NEGOTIATION: "border-[#d2c2aa] bg-[#f0e6d7] text-[#745f42]", WON: "border-[#bfcab5] bg-[#e5ebdf] text-[#4f6246]", LOST: "border-[#d4bcb4] bg-[#f0e1dc] text-[#7b5549]",
};
const TEMPERATURE_CLASS: Record<string, string> = { HOT: "border-[#d5b9ac] bg-[#f2e1da] text-[#7a5044]", WARM: "border-[#d6c6a8] bg-[#f0e7d5] text-[#735f3f]", COLD: "border-[#c9c8bd] bg-[#ecebe4] text-[#64645a]" };
...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<{ agent?: string; source?: string; provider?: string; campaign?: string }> }) {
  ...
  const breached = leads.filter((lead) => getSlaStatus(lead) === "BREACHED").length;
  const hasFilters = Boolean(filters.agent || filters.source || filters.provider || filters.campaign);

  let scopeText = membership.role === "AGENT" ? "Tus leads asignados." : membership.role === "MANAGER" ? "Leads visibles para tu equipo." : "Todos los leads comerciales de tu organización.";

  return (
    <main className="min-h-screen p-6 md:p-8 lg:p-10">
      <div className="mx-auto max-w-[1500px]">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div><p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[#8d7553]">Gestión comercial</p><h1 className="mt-3 font-serif text-4xl font-medium tracking-tight text-[#292722] md:text-5xl">Leads</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#625d55]">{scopeText}</p><p className="mt-2 text-xs text-[#8a8176]">Alcance: {ROLE_LABELS[membership.role] || membership.role}</p></div>
          <div className="flex gap-2">{hasFilters && <Link href="/protected/leads" className="rounded-lg border border-[#cdbfa9] bg-[#fffaf2] px-4 py-3 text-sm font-semibold text-[#5b5144]">Limpiar filtros</Link>}<Link href="/protected/leads/new" className="inline-flex items-center gap-2 rounded-lg bg-[#302d28] px-5 py-3 text-sm font-semibold !text-[#fffaf2]"><Plus size={16} />Nuevo lead</Link></div>
        </div>
        ...
        <section className="mt-6 overflow-hidden rounded-2xl border border-[#d2c5b3] bg-[#f7f0e6]">
          <div className="border-b border-[#ddd1c0] px-5 py-4 md:px-6"><div className="flex items-center justify-between gap-3"><div><h2 className="font-serif text-2xl text-[#302d28]">Cartera comercial</h2><p className="mt-1 text-xs text-[#81796e]">Origen, responsable, velocidad y próxima acción en una sola vista.</p></div><Link href="/protected/pipeline" className="inline-flex items-center gap-2 text-sm font-semibold text-[#6e604e]">Ver pipeline <ArrowRight size={15} /></Link></div></div>
          <div className="overflow-x-auto"><table className="w-full min-w-[1450px] text-left text-sm">
            ...
            <tbody>{leads.map((lead) => {
              const stage = lead.pipeline_stage || "NEW"; const temperature = lead.lead_temperature || ""; const sla = getSlaStatus(lead); const minutes = responseMinutes(lead);
            ...
            })}{!leads.length && !error && <tr><td colSpan={9} className="px-6 py-14 text-center"><UserRoundCheck className="mx-auto text-[#a08d72]" /><p className="mt-3 font-serif text-xl text-[#4b443a]">No hay leads con estos filtros.</p></td></tr>}</tbody>
          </table></div>
        </section>
      </div>
    </main>
...
function formatMoney(value: number | string | null, currency: string | null) { if (value == null || value === "") return "—"; const numeric = Number(value); return Number.isNaN(numeric) ? "—" : `${currency || ""} ${numeric.toLocaleString("es-UY")}`.trim(); }
function unique(values: (string | null)[]) { return Array.from(new Set(values.filter((value): value is string => Boolean(value)))).sort((a,b) => a.localeCompare(b,"es")); }
