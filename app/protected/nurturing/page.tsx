...
export default async function NurturingPage() {
  ...
  const active = (enrollments || []).filter((item: any) => item.status === "ACTIVE").length;
  const paused = (enrollments || []).filter((item: any) => item.status.startsWith("PAUSED_")).length;
  const blocked = (actions || []).filter((item: any) => item.status === "BLOCKED_META").length;
...
function relationName(value: any) { return Array.isArray(value) ? value[0]?.full_name : value?.full_name; }
