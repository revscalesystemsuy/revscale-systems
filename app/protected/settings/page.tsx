      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-8 grid gap-5 md:grid-cols-2">
          ...
          <Card title={<SectionTitle icon={Users}>Equipo</SectionTitle>}>
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="font-serif text-4xl font-medium text-[#302d28]">{membersCount ?? 0}</p>
                <p className="mt-1 text-sm text-[#70685e]">miembros activos</p>
              </div>
          ...
          <Card title={<SectionTitle icon={CreditCard}>Suscripción</SectionTitle>}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm text-[#70685e]">Plan actual</p>
                <p className="mt-1 text-lg font-semibold text-[#37332d]">{subscription?.plan || "TRIAL"}</p>
              </div>
              <span className="rounded-full border border-[#d5c8b7] bg-[#eee4d5] px-3 py-1 text-xs font-semibold text-[#625846]">{subscription?.status || "INACTIVE"}</span>
            </div>

            <div className="mt-5 border-t border-[#ddd1c0] pt-5">
              <h3 className="text-sm font-semibold text-[#514a42]">Uso actual</h3>
              <div className="mt-4 space-y-3 text-sm text-[#70685e]">
                <UsageRow icon={<Users size={14} strokeWidth={1.7} />} label="Agentes" value={`${membersCount ?? 0}/${subscription?.max_agents ?? 0}`} />
                <UsageRow icon={<UserRound size={14} strokeWidth={1.7} />} label="Leads" value={`${leadsCount ?? 0}/${subscription?.max_leads ?? 0}`} />
                <UsageRow icon={<Building2 size={14} strokeWidth={1.7} />} label="Propiedades" value={`${propertiesCount ?? 0}/${subscription?.max_properties ?? 0}`} />
              </div>
            </div>
        ...
        <section className="mt-8 rounded-2xl border border-[#d2c5b3] bg-[#f7f0e6] p-6 md:p-7">
          <div className="flex items-start gap-3">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-[#d2c5b3] bg-[#eee4d5] text-[#786447]"><Settings2 size={17} strokeWidth={1.7} /></span>
            <div>
              <h2 className="font-serif text-2xl font-medium text-[#37332d]">Capacidades de administración</h2>
              <p className="mt-2 text-sm leading-6 text-[#665f56]">Accesos disponibles según tu plan y rol. Las funciones bloqueadas siguen gestionándose desde Mi Plan.</p>
            </div>
          </div>
...
function Card({ title, children }: { title: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-[#d2c5b3] bg-[#f7f0e6] p-6 shadow-[0_18px_45px_rgba(72,58,40,0.04)]">
      <h2 className="mb-5 text-lg font-semibold text-[#37332d]">{title}</h2>
      {children}
    </div>
  );
}

function UsageRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="flex items-center justify-between gap-4"><span className="flex items-center gap-2">{icon}{label}</span><span className="font-semibold text-[#4b4338]">{value}</span></div>;
}

function Capability({ icon, label, href }: { icon: React.ReactNode; label: string; href: string }) {
  return <Link href={href} className="flex items-center justify-between gap-4 rounded-xl border border-[#ddd1c0] bg-[#fffaf2] px-4 py-3 text-sm font-medium text-[#514a42] transition hover:bg-[#f2e9dc]"><span className="flex items-center gap-2">{icon}{label}</span><span className="text-[#9a8e7d]">→</span></Link>;
}
