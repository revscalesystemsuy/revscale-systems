...
export async function updateTeam(formData: FormData) {
  const { organizationId, role, teamId: managerTeamId, admin } = await getEnterpriseManagerContext();
...
export async function deleteTeam(formData: FormData) {
  ...
  const { data: team } = await admin.from("teams").select("id,name").eq("id", teamId).eq("organization_id", organizationId).maybeSingle();
  ...
  const [{ count: membersCount }, { count: leadsCount }] = await Promise.all([
    admin.from("organization_members").select("id", { count: "exact", head: true }).eq("organization_id", organizationId).eq("team_id", teamId),
    admin.from("leads").select("id", { count: "exact", head: true }).eq("organization_id", organizationId).eq("team_id", teamId),
  ]);

  if ((membersCount || 0) > 0 || (leadsCount || 0) > 0) throw new Error("Mové primero los agentes y leads de este equipo antes de eliminarlo.");

  const { error } = await admin.from("teams").delete().eq("id", teamId).eq("organization_id", organizationId);
  if (error) throw new Error(error.message);
  revalidatePath("/protected/teams");
}
...
export async function updateMemberTeam(formData: FormData) {
  ...
  const { error } = await admin.from("organization_members").update({ team_id: teamId || null }).eq("id", memberId).eq("organization_id", organizationId);
  if (error) throw new Error(error.message);
  revalidatePath("/protected/teams");
...
export async function updateMemberRole(formData: FormData) {
  ...
  if (!["OWNER", "MANAGER", "AGENT"].includes(nextRole)) throw new Error("Rol inválido.");

  const { error } = await admin.from("organization_members").update({ role: nextRole }).eq("id", memberId).eq("organization_id", organizationId);
  if (error) throw new Error(error.message);
  revalidatePath("/protected/teams");
  revalidatePath("/protected/agents");
}
