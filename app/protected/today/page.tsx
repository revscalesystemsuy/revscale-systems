...
export default async function TodayPage() {
  ...
  const [{ data: leadsData }, { data: interactionsData }, { data: followupsData }, { data: slaSettings }] = await Promise.all([
    supabase
      .from("leads")
      .select("id,full_name,phone,pipeline_stage,stage_entered_at,expected_close_date,lead_temperature,requires_human,next_action,created_at,budget_max,currency,source_channel,source_provider,source_campaign,assigned_at,first_human_response_at,sla_deadline,sla_breached_at")
    ...
    supabase
      ...
      .select("is_enabled,warning_minutes_before")
  ...
  const slaQueue = slaEnabled
    ? (leadsData || [])
        .filter((lead) => OPEN_PIPELINE_STAGE_SET.has(lead.pipeline_stage || "NEW") && lead.sla_deadline && !lead.first_human_response_at)
        .map((lead) => ({ ...lead, slaStatus: getSlaStatus(lead, now, warningMinutes) }))
        .filter((lead) => ["WAITING", "WARNING", "BREACHED"].includes(lead.slaStatus))
        .sort((a, b) => {
          const rank = { BREACHED: 3, WARNING: 2, WAITING: 1 } as const;
          const statusDiff = rank[b.slaStatus as keyof typeof rank] - rank[a.slaStatus as keyof typeof rank];
    ...
    <main className="min-h-screen p-6 md:p-8 lg:p-10">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <Metric title="SLA vencido" value={slaBreached} />
        ...
        {slaEnabled && (
          <section className="mt-7 rounded-xl border border-[#cbb99f] bg-[#efe3d3] p-6">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#80694b]">Velocidad de primera respuesta</p>
                <h2 className="mt-2 font-serif text-2xl font-medium text-[#37332d]">SLA que requiere atención</h2>
                <p className="mt-2 text-sm leading-6 text-[#71675b]">La respuesta de IA no cierra este reloj: RevScale espera la primera respuesta humana.</p>
              </div>
              <Link href="/protected/settings/sla" className="text-sm font-medium text-[#725d40]">Ver configuración SLA</Link>
            </div>

            <div className="mt-5 space-y-3">
              {slaQueue.map((item) => (
                <article key={item.id} className={`rounded-xl border p-4 ${item.slaStatus === "BREACHED" ? "border-[#b88e75] bg-[#f1dfd2]" : item.slaStatus === "WARNING" ? "border-[#c4a86e] bg-[#f3e8cf]" : "border-[#d7caba] bg-[#fffaf2]"}`}>
                  <div className="grid gap-4 lg:grid-cols-[1.2fr_1fr_160px_150px] lg:items-center">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <Link href={`/protected/leads/${item.id}`} className="font-semibold text-[#37332d] hover:text-[#725d40]">{item.full_name || "Sin nombre"}</Link>
                        {item.lead_temperature === "HOT" && <span className="rounded-full border border-[#d5b9ac] bg-[#f2e1da] px-2 py-0.5 text-[10px] font-semibold text-[#7a5044]">HOT</span>}
                      </div>
                      <p className="mt-1 text-xs text-[#81796e]">{item.source_provider || item.source_channel || "Origen sin atribuir"}{item.source_campaign ? ` · ${item.source_campaign}` : ""}</p>
                    </div>
                    <div>
                      <p className="text-xs uppercase tracking-[0.12em] text-[#81796e]">Vencimiento</p>
                      <p className="mt-1 text-sm font-medium text-[#4f493f]">{formatDateTime(item.sla_deadline)}</p>
                    </div>
                    <SlaPill status={item.slaStatus} />
                    <div className="flex gap-2">
                      <WhatsAppButton leadId={item.id} phone={item.phone} compact />
                      <Link href={`/protected/leads/${item.id}`} className="inline-flex items-center justify-center rounded-lg border border-[#cdbfa9] bg-[#fffaf2] px-3 py-2 text-xs font-semibold text-[#554f47]">Abrir</Link>
                    </div>
                  </div>
                </article>
              ))}
              {!slaQueue.length && <p className="rounded-xl border border-[#d7caba] bg-[#fffaf2] p-4 text-sm text-[#81796e]">No hay leads esperando primera respuesta humana dentro del SLA.</p>}
            </div>
          </section>
        )}
...
function SlaPill({ status }: { status: "WAITING" | "WARNING" | "BREACHED" }) {
  const label = status === "BREACHED" ? "Incumplido" : status === "WARNING" ? "Por vencer" : "En SLA";
  const className = status === "BREACHED" ? "border-[#b88e75] bg-[#ead3c3] text-[#6b4433]" : status === "WARNING" ? "border-[#c4a86e] bg-[#eee2c8] text-[#6f5a2e]" : "border-[#d2c5b3] bg-[#eee4d5] text-[#6f6558]";
  return <span className={`inline-flex w-fit rounded-full border px-3 py-1 text-xs font-semibold ${className}`}>{label}</span>;
}
