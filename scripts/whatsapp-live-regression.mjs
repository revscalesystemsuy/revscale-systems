...
const checks = [
  ...
  [webhook.includes('return { ...outbound, error_code: "PROVIDER_STATE_UNKNOWN"'), "Ambiguous AI delivery must surface the unknown state to handoff logic"],
  [webhook.includes("El estado de una respuesta automática quedó incierto"), "Ambiguous AI delivery must force human handoff"],
  ...
  [sender.includes('message.status === "FAILED"'), "Human sender must distinguish explicit provider failure"],
  [sender.includes('.eq("status", "FAILED")'), "Explicit provider failure retry must be claimed atomically"],
  [sender.includes("PROVIDER_STATE_UNKNOWN"), "Human sender must preserve ambiguous delivery state"],
  [sender.includes("No lo reenviamos automáticamente para evitar duplicados"), "Ambiguous human sends must never auto-resend"],
