...
begin
  ...
  if coalesce(req.status, '') <> 'PENDING' then
  ...
  if requested_plan not in ('STARTER','PROFESSIONAL','ENTERPRISE') then
    raise exception 'Plan inválido';
  end if;

  requested_rank := case requested_plan
    when 'STARTER' then 1
  ...
  if target_user_id is null then
    raise exception 'El cliente todavía no creó su cuenta. Pedile que se registre con el mismo email de la solicitud.';
  end if;
  ...
  if req.organization_id is not null then
    ...
    where om.user_id = target_user_id and om.status = 'ACTIVE';
    ...
    if membership_count > 1 then
      ...
      into target_org_id, existing_role
      from public.organization_members om
      where om.user_id = target_user_id and om.status = 'ACTIVE'
      limit 1;

      if existing_role <> 'OWNER' then
        raise exception 'La cuenta ya pertenece a una organización pero no tiene rol Director.';
      end if;
    else
      insert into public.organizations(name, slug)
      values (
        coalesce(nullif(trim(req.company),''),'Nueva inmobiliaria'),
        ''
      )
      ...
      insert into public.organization_onboarding(organization_id,completed)
      values (target_org_id,false);
    end if;
  end if;

  select upper(s.plan) into current_plan
  ...
  if current_rank > requested_rank then
    raise exception 'La organización ya tiene un plan superior (%). No se realizó ninguna baja.', current_plan;
  end if;
  ...
  set plan = requested_plan,
      status = 'ACTIVE',
      max_agents = limits_agents,
      max_leads = limits_leads,
      max_properties = limits_properties,
      updated_at = now()
  where organization_id = target_org_id;

  if not found then
    insert into public.subscriptions(
      organization_id,plan,status,max_agents,max_leads,max_properties,updated_at
    ) values (
      target_org_id,requested_plan,'ACTIVE',limits_agents,limits_leads,limits_properties,now()
    );
  end if;
  ...
  if target_team_id is null then
    ...
    from public.teams t
    where t.organization_id = target_org_id and lower(t.name) = lower('Equipo Principal')
  ...
  set team_id = coalesce(team_id,target_team_id)
  where organization_id = target_org_id and user_id = target_user_id;

  if not exists (
    select 1 from public.organization_onboarding oo where oo.organization_id = target_org_id
  ) then
    insert into public.organization_onboarding(organization_id,completed)
    values (target_org_id,false);
  end if;
...
revoke all on function public.platform_admin_activate_plan_request(uuid) from public;
