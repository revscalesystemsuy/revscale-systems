alter function public.process_paddle_billing_event(text,text,uuid,text,text,text,text,timestamptz,boolean,text)
  rename to process_paddle_billing_event_initial;
...
begin
  if p_subscription_id is not null and p_price_id is not null
    ...
    if v_change.id is not null then
      ...
      select count(*) into v_agents from public.organization_members
      where organization_id=v_change.organization_id and status='ACTIVE';
      select count(*) into v_leads from public.leads where organization_id=v_change.organization_id;
      select count(*) into v_properties from public.properties where organization_id=v_change.organization_id;

      if v_agents > v_limit_agents or v_leads > v_limit_leads or v_properties > v_limit_properties then
        update public.subscription_change_requests
        set status='FAILED',processed_at=now(),error_text='La organizacion supera los limites del plan solicitado'
        where id=v_change.id;
        update public.billing_events
        set processed_at=now(),error_text='Subscription changed at provider but local limits no longer allow target plan'
        where id=v_event_id;
        return jsonb_build_object('ok',false,'changed',false,'reason','local_limits_changed');
      end if;
...
revoke all on function public.process_paddle_billing_event_initial(text,text,uuid,text,text,text,text,timestamptz,boolean,text)
  from public,anon,authenticated;
grant execute on function public.process_paddle_billing_event_initial(text,text,uuid,text,text,text,text,timestamptz,boolean,text)
  to service_role;
